...
public class ServerServices : IServerServices
{
    ...
    public List<DeviceInfo> Devices { get => devices; }
    ...
    {
        var response = _httpClient.GetAsync("/Device/GetDevice").Result;
        HttpStatusCode statusCode = response.StatusCode;

        if (statusCode == HttpStatusCode.OK)
        {
            var responseContent = response.Content.ReadAsStringAsync().Result;
            devices = JsonSerializer.Deserialize<List<DeviceInfo>>(responseContent);
        }
    }
    public void ConnectDevices()
    {
        ...
        {
            (bool isDone, string status) = device.ZktServices.ConnectByIp(device.Ip, device.Port);
            _logger.LogInformation("Device Connection is {status}: {deviceIsConnect}  ,  at : {time}", status, device.ZktServices.IsConnect(), DateTimeOffset.Now);
